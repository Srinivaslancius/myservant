...
if(isset($_POST["submit"]) && $_POST["submit"]!="") {
	...
	if($_POST['payment_group'] == "1") {
		for($i=0;$i<$servicesCount;$i++) {
			...
			$orders = "INSERT INTO services_orders (`first_name`, `last_name`, `company_name`, `email`, `mobile`, `address`, `country`, `postal_code`, `city`, `order_note`, `category_id`, `sub_category_id`,  `group_id`, `service_id`, `service_price_type_id`,`service_price`,`service_quantity`, `service_selected_date`, `service_selected_time`, `sub_total`, `order_total`, `coupon_code`, `payment_method`,`service_tax`, `order_id`,`order_sub_id`, `created_at`) VALUES ('$first_name','$last_name', '$company_name','$email','$mobile','$address','$country','$postal_code','$city','$order_note','" . $_POST["category_id"][$i] . "','" . $_POST["sub_cat_id"][$i] . "','" . $_POST["group_id"][$i] . "','" . $_POST["service_id"][$i] . "','" . $_POST["service_price_type_id"][$i] . "','" . $_POST["service_price"][$i] . "','" . $_POST["service_quantity"][$i] . "','" . $_POST["service_selected_date"][$i] . "','" . $_POST["service_selected_time"][$i] . "','$sub_total','$order_total','$coupon_code','$payment_group','$service_tax', '$order_id','$sub_order_id','$order_date')";
